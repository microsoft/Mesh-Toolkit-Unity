...
namespace MeshApp.Animations
{
    ...
    [UserCreatable(false)]
    public class BoatControllerAnimator : AnimationNode
    {
        private readonly float[] _baseLayerSpeeds = { -1F, 1F, };

        protected BoatControllerAnimator(in Guid ahandle, bool transfer)
        ...
        internal static BoatControllerAnimator GetOrCreateInstance(in Guid cookie, bool transfer)
        {
            var result = GetOrCreateDomObject(
                cookie,
                transfer,
                (handle, t) => new BoatControllerAnimator(handle, transfer: t));

            return result as BoatControllerAnimator;
        }
    }
}